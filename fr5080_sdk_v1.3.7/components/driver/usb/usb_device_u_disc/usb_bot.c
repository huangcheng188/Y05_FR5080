...
uint8_t *Bulk_Data_Buff;//使用malloc申请内存
...
void Mass_Storage_In (void)
{
    ...
    switch (Bot_State)
    {
        case BOT_CSW_Send:
        case BOT_ERROR:
            if(is_stall)        //dyc added。 csw wait until stall handshake is send
            {
                is_stall= 0;
            ...
            Bot_State = BOT_IDLE;
            SetEPRxStatus(USBMASS_IN_EP, EP_RX_VALID);/* enable the Endpoint to receive the next cmd*/
            if (GetEPRxStatus(USBMASS_IN_EP) == EP_RX_STALL)
            {
                SetEPRxStatus(USBMASS_IN_EP, EP_RX_VALID);/* enable the Endpoint to receive the next cmd*/
            }
            break;
        ...
        case BOT_DATA_IN_LAST:
            ...
            SetEPRxStatus(USBMASS_IN_EP, EP_RX_VALID);
            break;
        default:
            break;
    }
...
* Return         : None.
*******************************************************************************/
void Mass_Storage_Out (uint8_t *buff,uint32_t size)
{
    uint8_t CMD;
    USB_STATUS_REG|=0X10;//标记轮询
    CMD = CBW.CB[0];
    ...
    Data_Len = size;
    Bulk_Data_Buff = buff;
    //show_reg(Bulk_Data_Buff,Data_Len,1);
    switch (Bot_State)
    {
        case BOT_IDLE:
            CBW_Decode();
...
{
    USB_SIL_Write(USBMASS_OUT_EP, Data_Pointer, Data_Len);
...
void Set_CSW (uint8_t CSW_Status, uint8_t Send_Permission)
{
    ...
    USB_SIL_Write(USBMASS_OUT_EP, ((uint8_t *)& CSW), CSW_DATA_LENGTH);
...
void Bot_Abort(uint8_t Direction)
{
    ...
    {
        case DIR_IN :
            SetEPTxStatus(USBMASS_OUT_EP, EP_TX_STALL);
            is_stall = 1;
            break;
        case DIR_OUT :
            SetEPRxStatus(USBMASS_IN_EP, EP_RX_STALL);
            break;
        case BOTH_DIR :
            SetEPTxStatus(USBMASS_OUT_EP, EP_TX_STALL);
            is_stall = 1;
